...
import time
import os
import cv2  # <-- ADDED for saving PNGs including depth
...
for i in range(num_images):
    ...
    # Convert depth to uint16 (in millimeters) and save as PNG
    depth_left_img_mm = (depth_left_img * 1000.0).astype(np.uint16)
    ...
    depth_left_filename = os.path.join(depth_left_dir, f"depth_{i:06d}.png")
    ...
    cv2.imwrite(depth_left_filename, depth_left_img_mm)

    # Print saved file info every second
    if i % fps == 0:
        print(f"Saved RGB and Depth frame: {i}")

    # Record ground-truth pose in TUM format
