...
def capture_nonros_depth_rgb_data():
    # Connect to AirSim
    ...
    depth_dir = os.path.join(save_dir, "depth_planar")
    ...
    semantic_txt = open(os.path.join(save_dir, "semantic.txt"), "w")
    ...
    association_segm_txt = open(os.path.join(save_dir, "associations_segm.txt"), "w")
    ...
    for i in range(num_frames):
        ...
        responses = client.simGetImages([
            ...
            airsim.ImageRequest("front_left_custom", airsim.ImageType.DepthPlanar, pixels_as_float=True),
        ...
        if len(responses) != 3 or any(r.height == 0 for r in responses):
            print(f"Frame {i}: Empty image, skipping.")
            continue

        # Decode RGB
        rgb_resp = responses[0]
        rgb_img = np.frombuffer(rgb_resp.image_data_uint8, dtype=np.uint8).reshape(rgb_resp.height, rgb_resp.width, 3)

        # Decode Depth，when pixel as float is true
        depth_resp = responses[1]
        depth_img = np.array(depth_resp.image_data_float, dtype=np.float32).reshape(depth_resp.height, depth_resp.width)
        depth_img_mm = (depth_img * 1000).astype(np.uint16)

        # Decode Segmentation
        semantic_resp = responses[2]
        seg_img = np.frombuffer(semantic_resp.image_data_uint8, dtype=np.uint8).reshape(semantic_resp.height,
                                                                                            semantic_resp.width, 3)

        # Filenames (timestamp as name)
        ts_str = f"{timestamp:.6f}"
        rgb_filename = f"{ts_str}.png"
        depth_filename = f"{ts_str}.png"
        seg_filename = f"{ts_str}.png"

        # Save images
        airsim.write_png(os.path.join(rgb_dir, rgb_filename), rgb_img)
        cv2.imwrite(os.path.join(depth_dir, depth_filename), depth_img_mm)
        airsim.write_png(os.path.join(semantic_dir, seg_filename), seg_img)

        # Log timestamps
        rgb_txt.write(f"{ts_str} rgb/{rgb_filename}\n")
        depth_txt.write(f"{ts_str} depth/{depth_filename}\n")
        semantic_txt.write(f"{ts_str} semantic/{seg_filename}\n")

        # Log associations
        association_segm_txt.write(
            f"{ts_str} rgb/{rgb_filename} {ts_str} depth/{depth_filename} {ts_str} semantic/{seg_filename}\n")
        association_txt.write(
            f"{ts_str} rgb/{rgb_filename} {ts_str} depth/{depth_filename} {ts_str}\n")
    ...
    print("✅ Done! TUM-format dataset with semantic saved at:", save_dir)


if __name__ == "__main__":
    capture_nonros_depth_rgb_data()
